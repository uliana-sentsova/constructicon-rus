...
xml = open("konstruktikon-rus.xml")
...
#     Inconsistent name
    ...
    if inconsistent:
        out.write("Inconsistent tokens (name): {}.".format(", ".join(inconsistent)) + "\t")
    if " " in c.name:
    ...
    if def_rus_inconsistent:
        out.write("Inconsistent tokens (definition rus): {}.".format(", ".join(def_rus_inconsistent)) + "\t")

    def_eng_inconsistent = consistent(def_eng_annotation, allowed, illegal)
    if def_eng_inconsistent:
        out.write("Inconsistent tokens (definition eng): {}.".format(", ".join(def_eng_inconsistent)) + "\t")

#     Inconsistent examples
    if len(c.examples) < 3:
        out.write("Insufficient number of examples ({} more needed).".format(3 - len(c.examples)) + "\t")
    ...
    if examples_inconsistent:
        out.write("Inconsistent tokens (examples): {}".format(", ".join(examples_inconsistent)) + "\t")
